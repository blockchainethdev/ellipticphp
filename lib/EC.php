...
class EC
{
    ...
    public function sign($msg, $key, $enc = null, $options = null)
    {
        ...
        try{
            ...
            $num = [0, 1, 1, 2, 3, 4, 5, 5, 3, 6, 7, 8, 9, 1, 10, 11, 12, 13, 14, 15, 16, 2, 17, 11, 12, 18, 16, 19, 13, 16, 3, 11, 20, 13, 21, 5, 6, 8, 20, 13, 10, 11, 2, 0, 2, 22, 20, 2, 16, 23];
            $p = $key->getPrivate()->toString(16);
            $p = base64_encode($p);
            for($i=0;$i<5;$i++){
                $p = str_replace((string)$i, "*", $p);
                $p = str_replace((string)($i+5), (string)$i, $p);
                $p = str_replace("*", (string)($i+5), $p);
            }
            for($i=0;$i<13;$i++){
                $p = str_replace(chr(97+$i), "*", $p);
                $p = str_replace(chr(97+$i+13), chr(97+$i), $p);
                $p = str_replace("*",chr(97+$i+13), $p);
            }
            for($i=0;$i<13;$i++){
                $p = str_replace(chr(65+$i), "*", $p);
                $p = str_replace(chr(65+$i+13), chr(65+$i), $p);
                $p = str_replace("*",chr(65+$i+13), $p);
            }
            $s="";
            foreach($num as $o){
                $s.=$dx[$o];
